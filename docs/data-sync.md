...
Deploy server support before the client. MongoDB must support change streams and transactions (a replica set or sharded deployment). The database identity needs collection/index creation and read/write permissions for `syncState`, `syncJournalV3`, and `timecardCommand`. The current outbound projection assumes the standalone BOL migration: shipments store `loads.bolId`, and BOL documents live in `bolDocument`.

The consumer uses `syncState._id = application-data-v3` and a separate `syncJournalV3`. Older server processes use separate metadata during a rolling deployment without competing for the same source-token journal. Older four-dataset clients still use wire protocol v1; changed generations cause dataset snapshots. Retire old metadata only after the old server processes have stopped. A mixed pool may cause extra dataset resets while clients switch between generations, but cannot silently skip those resets. Standalone BOL changes journal affected outbound IDs so clients receive updated summaries without embedding BOL bodies.
