...
The final heartbeat arrived at **12:44 Eastern**, after the requested 08:00 cutoff. The automation was explicitly **paused** at completion. Saved checks exist around 01:32, 02:00/recovery, 02:31, and 12:44; there is no saved continuous observation between 02:31 and 12:44. The final retained metric window starts at 08:55, so neither the missing interval nor the entire night can be certified healthy.

Release **v133** (`88a59ae`) remains deployed. At 12:44, `/health` returned HTTP 200 and sync retained fence 151, a valid lease, no capture error, and approximately **2.8 seconds** of source/heartbeat age. There were still no journal entries since the original performance release for the subscribed business datasets, and the retrieved application logs contained no approval/import timing samples. Production latency improvement under active workload remains unverified.

**Memory pressure is unresolved.** The retained logs contain **57 R14 errors**, with the first in that window at 11:27:15 and the latest at 12:44:13. Latest total memory was **530.40 MB**, comprising **462.84 MB RSS**, approximately **1.98 MB cache**, and **65.58 MB swap**, against the reported **512 MB quota**. Observed peak total memory in the retained window was **542.52 MB**. No R15, new capture lease failure, or router crash error appeared in that retained window. The healthy endpoint and capture lease do not negate this resource regression.

The next performance investigation should measure retained heap and memory changes around recurring background jobs and representative read traffic. The five-minute invoice/ASN work remains a candidate, not an established cause. A production diagnostic change or infrastructure resize still requires approval; no additional deployment, restart, resize, or business-record test was performed during the final check. Evidence is saved in workspace `tmp/performance-monitor/20260913-1644-final/capture.json` and `tmp/performance-final-metrics.json`.

## Findings and changes

### Overnight observations

- **02:04 Eastern:** The 02:00 check detected HTTP 503/H10 and an expired sync lease (last heartbeat approximately 5.6 minutes old). Production had been replaced by **v132**, commit `66e5a9c`, at 01:55:23. Its socket connection handler crashed while requiring `socket/event/messageAttachment.js:44`: `await getConfiguredDropbox()` was inside a non-async `storage` function. The deployed source reproduced the syntax failure with `node --check`; this was not a measured memory-quota failure. Concurrent recovery work committed `88a59ae` and deployed **v133** at 02:02:41 without intervention from this monitor. This monitor verified its syntax and 16 message-center tests, then confirmed HTTP 200, live socket connections, and a healthy sync lease with fence 151 and approximately 2.4 seconds of source age at 02:03:46. Before the outage, memory peaked at 458.67 MB, with no swap or R14/R15 observed. No post-release import/approval timing samples or business-change journal entries were available. Recovery snapshots are under `tmp/performance-monitor/20260913-0600/`. The local log checker now also recognizes router crash/timeout codes and crashed process transitions. Future observations must account for the v133 restart instead of treating its memory reset or fence increment as a v131 performance result.
