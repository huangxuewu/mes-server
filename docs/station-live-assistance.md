...
STUN uses the existing `SHARING_STUN_URLS` setting (default `stun:stun.l.google.com:19302`). For routed networks requiring a relay, configure a TURN service using `STATION_LIVE_TURN_URLS` (comma-separated `turn:`/`turns:` URLs), `STATION_LIVE_TURN_USERNAME`, and `STATION_LIVE_TURN_CREDENTIAL`. These credentials are delivered only after session authorization; use credentials intended for clients. Without a working direct or relay route, Live falls back to JPEG. No TURN service is provisioned by this change. SDP and media are not logged by the implementation.
