...
const { buildAsn, transactionState, submitAsns, ASN_QUERY, ACCOUNT_QUERY, CREATE_ASN, SAVE_ADJUSTMENTS, SAVE_BOL } = require("../utils/edi/asn");
...
const transport = (loadNumber, { uploadFailure = false, createFailure = false, pending = false } = {}) => {
    ...
    const client = {
        ...
        async graphql(query, variables) {
            ...
            if (query === SAVE_ADJUSTMENTS) return { upsertLoadShipmentAdjustments: [{ id: 1 }] };
            if (query === SAVE_BOL) return { upsertLoad: { id: 20, bol_number: variables.input.bol_number } };
            throw new Error("Unexpected GraphQL request");
        },
        async uploadBol(bytes, load, bol) {
            calls.push({ query: "upload", bytes, load, bol });
            if (uploadFailure) { uploadFailure = false; throw new Error("ERP upload failed"); }
            return { path: `/BOL/${load}#_${bol}#.pdf` };
        },
    };
    return { mes, shipment, calls, client, options: { clientFactory: async () => client } };
};

test("ERP receipt, BOL upload and ERP load update happen in order", async () => {
    const f = transport("2001");
    const progress = [];
    const result = await submitAsns({ shipments: [f.mes], file }, { ...f.options, onProgress: value => progress.push(value) });
    const operations = f.calls.map(call => call.query);
    assert.ok(operations.indexOf(CREATE_ASN) < operations.indexOf("upload"));
    assert.ok(operations.indexOf(SAVE_ADJUSTMENTS) < operations.indexOf("upload"));
    assert.equal(operations.at(-1), SAVE_BOL);
    ...
    assert.equal(progress.at(-1).phase, 'uploadingBol');
    assert.equal(progress.at(-1).erpLoadId, 20);
    assert.equal(result.erpLoadId, 20);
    assert.equal(f.calls.find(call => call.query === SAVE_BOL).variables.input.id, 20);
...
test("ERP upload failure retries without resending an accepted ASN", async () => {
    const f = transport("2002", { uploadFailure: true });
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /ERP upload failed/);
    await submitAsns({ shipments: [f.mes], file }, f.options);
    assert.equal(f.calls.filter(call => call.query === CREATE_ASN).length, 1);
    assert.equal(f.calls.filter(call => call.query === "upload").length, 2);
});

test("ERP receipt finishes while validation, delivery and partner acknowledgement are pending", async () => {
    const f = transport("2003", { pending: true, uploadFailure: true });
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /ERP upload failed/);
    const result = await submitAsns({ shipments: [f.mes], file }, f.options);
    assert.equal(result.transactions[0].id, "transaction1");
...
test("uncertain creation is not repeated when ERP has not exposed its outcome", async () => {
    ...
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /response lost/);
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /outcome is unknown/);
    assert.equal(f.calls.filter(call => call.query === CREATE_ASN).length, 1);
    assert.equal(f.calls.filter(call => call.query === "upload").length, 0);
});

test("missing ERP transaction ID cannot be treated as receipt or reach BOL upload", async () => {
    ...
    const progress = [];
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, { ...f.options, onProgress: value => progress.push(value) }), /transaction ID/);
...
test("validate all loaded POs before any create, and detect conflicting existing ASN", async () => {
    ...
    await assert.rejects(submitAsns({ shipments: [f.mes, { ...f.mes, shipmentId: "missing" }], file }, f.options), /matching ERP shipment/);
    assert.equal(f.calls.filter(call => call.query === CREATE_ASN).length, 0);
    await submitAsns({ shipments: [f.mes], file }, f.options);
    f.mes.bol.number = "99999";
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /different BOL or quantities/);
    assert.equal(f.calls.filter(call => call.query === CREATE_ASN).length, 1);
});

test("multiple loaded POs each receive an ASN, followed by one load BOL upload", async () => {
    ...
    const client = {
        ...
        async graphql(query, variables) {
            ...
            if (query === SAVE_BOL) return { upsertLoad: variables.input };
        },
        async uploadBol(...args) { uploaded.push(args); },
    };
    const result = await submitAsns({ shipments: [first.mes, second.mes], file }, { clientFactory: async () => client });
    assert.deepEqual(created, ["1", "2"]);
    assert.equal(uploaded.length, 1);
    assert.equal(result.transactions.length, 2);
});

test("unloaded shipments and invalid PDFs cannot reach ERP writes", async () => {
    const f = transport("3002");
    f.mes.checklist.loaded.status = false;
    await assert.rejects(submitAsns({ shipments: [f.mes], file }, f.options), /load and upload/);
    await assert.rejects(submitAsns({ shipments: [f.mes], file: Buffer.from("not-pdf") }, f.options), /must be a PDF/);
    assert.equal(f.calls.filter(call => call.query === CREATE_ASN || call.query === "upload").length, 0);
});

test("row retries create only the selected PO and upload BOL after the remaining PO has a receipt", async () => {
    ...
    const request = { shipments: [first.mes, second.mes], file };
    ...
    assert.equal(firstResult.bolUploaded, false);
    ...
    assert.equal(secondResult.bolUploaded, true);
    assert.deepEqual(created, ['1', '2']);
    assert.equal(uploads.length, 1);
    assert.equal(secondResult.transactions.length, 2);
