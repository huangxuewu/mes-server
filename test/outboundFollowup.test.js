...
const fixture = (db, submitAsns) => {
    ...
    const dependencies = {
        ...
        '../../utils/outboundWorkflowState': { ...require('../utils/outboundWorkflowState'), createOutboundWorkflowState: () => ({ run: (_numbers, work) => work({}), readLoad: async () => [{ shipmentId: 'SHIP1', loadNumber: 'LOAD1', inspectionRelease: { id: 'signed', loadNumber: 'LOAD1' }, checklist: { labeled: { status: true }, inspected: { status: true } } }] }) },
    };
...
test('warehouse saves cannot overwrite hidden follow-up flags from a stale checklist', async () => {
    let update;
    const { handlers } = fixture({ outbound: {
        findOne: () => ({ session() { return this; }, lean: async () => ({ loads: [{ shipmentId: 'SHIP1', loadNumber: 'LOAD1', inspectionRelease: { id: 'signed', loadNumber: 'LOAD1' }, checklist: { labeled: { status: true }, inspected: { status: true } } }] }) }),
