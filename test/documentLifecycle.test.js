...
const fixture = (records, mutateAfterRead) => {
    ...
    vm.runInNewContext(source, { require: name => {
        if (name === './documentAccess') return { protectedDocumentEmitter: io => io };
        if (name === './memoryDiagnostics') return require('../utils/memoryDiagnostics');
        assert.equal(name, '../models'); return db;
    }, module, console, Date });
