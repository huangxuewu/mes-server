...
const fixture = () => {
    ...
    db.permissionRule = new Collection();
    ...
    const connect = userId => {
        ...
        load('socket/event/permissionRule.js')(socket, io);
...
test('rule assignment replaces personal permissions and login, roster and reconnect agree', async t => {
    ...
    const created = await admin.call('permissionRule:create', { name: 'Purchasing', permission: { view: ['purchase', 'purchase'], access: ['financial.page.access'] } });
    assert.equal(created.status, 'success');
    const ruleId = created.payload._id;
    assert.equal(created.payload.permission.view.length, 1);
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: ruleId })).status, 'success');
    ...
    assert.equal(login.payload.user.permissionRuleName, 'Purchasing');
...
test('editing one rule updates all bound members and revokes subscriptions without logout', async t => {
    const { db, connect, session } = fixture();
    ...
    const result = await admin.call('permissionRule:create', { name: 'Office', permission: { view: ['office.calendar.event.public.view'] } });
    await admin.call('user:update', { _id: ids.b, permissionRuleId: result.payload._id });
    ...
    assert.equal(worker.rooms.has(session.PUBLIC_EVENT_ROOM), true);
    const updated = await admin.call('permissionRule:update', { _id: result.payload._id, name: 'Office updated', permission: { view: ['replacement'] } });
    ...
    for (const client of [worker, otherStation]) {
        ...
        assert.equal(refreshed.permissionRuleName, 'Office updated');
        assert.equal(refreshed.password, undefined);
        assert.equal(client.received.some(item => item.event === 'permissionRules:changed'), false);
    }
    assert.equal(worker.socket.data.sessionGeneration, generation);
    assert.ok(admin.received.some(item => item.event === 'permissionRules:changed'));
});

test('only Admin manages rules and assignments; assigned users cannot receive individual overrides', async t => {
    ...
    for (const client of [worker, anonymous]) {
        assert.equal((await client.call('permissionRules:get', {})).status, 'error');
        assert.equal((await client.call('permissionRule:create', { name: 'Bad', permission: {} })).status, 'error');
    }
    const created = await admin.call('permissionRule:create', { name: 'Buyer', permission: { view: ['purchase'] } });
    const ruleId = created.payload._id;
    assert.equal((await worker.call('user:update', { _id: ids.b, permissionRuleId: ruleId })).status, 'error');
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: ids.outsider })).status, 'error');
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: ruleId, permission: { view: ['override'] } })).status, 'error');
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: ruleId })).status, 'success');
    ...
    assert.equal((await manager.call('permissionRule:update', { _id: ruleId, name: 'Escalate', permission: {} })).status, 'error');
    assert.equal((await admin.call('permissionRule:create', { name: '', permission: {} })).status, 'error');
    assert.equal((await admin.call('permissionRule:create', { name: 'Invalid', permission: { arbitrary: [] } })).status, 'error');
    assert.equal((await admin.call('permissionRule:create', { name: 'Invalid', permission: { view: [12] } })).status, 'error');
});

test('Admin can switch a rule user to explicitly chosen personal permissions in one save', async t => {
    const { connect, session } = fixture();
    const admin = connect(ids.a), worker = connect(ids.b);
    t.after(() => { admin.close(); worker.close(); });
    const rule = await admin.call('permissionRule:create', { name: 'Office', permission: { view: ['group.resource'] } });
    await admin.call('user:update', { _id: ids.b, permissionRuleId: rule.payload._id });
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: '', permission: { view: ['personal.resource'] } })).status, 'success');
    ...
    assert.equal((await admin.call('user:update', { _id: ids.b, permissionRuleId: rule.payload._id, permission: { view: ['personal.resource'] } })).status, 'error');
});

test('unassigned legacy accounts retain permissions; removal or missing rules never restore them', async t => {
    const { db, connect, session } = fixture();
    ...
    const rule = await admin.call('permissionRule:create', { name: 'Office', permission: { view: ['office'] } });
    await admin.call('user:update', { _id: ids.b, permissionRuleId: rule.payload._id });
    await admin.call('user:update', { _id: ids.b, permissionRuleId: '' });
