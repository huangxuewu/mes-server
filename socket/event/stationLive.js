...
module.exports = (socket, io) => {
    ...
    for (const action of ['start', 'frame', 'video', 'action', 'stop', 'remoteMessage', 'transfer']) socket.on(`station:live:${action}`, async (input, callback) => {
        ...
        try {
            ...
            callback({ status: 'success', payload: await live[action](socket, value, { frameProtocol: input.frameProtocol, videoProtocol: input.videoProtocol, displayId: input.displayId }) });
